...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 16; i++) {
			...
			EigenvalueFinder finder = new EigenvalueFinder(grid, v, 2.0d);

			for (int nOfDesiredNodes = 0; nOfDesiredNodes < 1; nOfDesiredNodes++) {
				EnergyLevel ek = finder.findEigenvalue(nOfDesiredNodes, grid, v);
				System.out.println(i + " " + grid.h + " " + ek.energy);
			}
		}
	}
