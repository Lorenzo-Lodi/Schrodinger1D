...
public class ShootingSolver {
    ...
    private double computeDerivativeMismatch(QuantumLevel level, int matchIndex) {
        ...
        for (int n = 1; n < matchIndex + 1; n++) {
            level.psi[n + 1] = integrator.propagate(level.psi, n, hy, level, Integrator.Direction.FORWARD);
        ...
        for (int n = np - 2; n > matchIndex - 1; n--) {
            level.psi[n - 1] = integrator.propagate(level.psi, n, hy, level, Integrator.Direction.BACKWARD);
